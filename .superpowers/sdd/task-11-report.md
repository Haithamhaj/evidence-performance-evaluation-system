...
## Nineteenth-review remediation

- Strict RED added seventeen prohibited fixtures and four safe controls covering exact `Function.prototype.call`/`apply`/`bind` aliases and meta-calls, bound `Reflect.apply`, instance/static/multi-level subclass receivers, returned lexical closures, and reflective mutations. The focused repository suite failed because sixteen prohibited fixtures were accepted; one existing conservative path rejected the seventeenth. The allowed suite passed after removing an unrelated protected `generate` name from one local-only control.
- Call normalization now preserves the effective target, runtime receiver, bound arguments, and exact invocation position through direct, computed, destructured, optional, meta-call, and bound forms. Unknown argument lists, receiver targets, cycles, and expansion caps remain fail-closed while safe-after ordering and cross-subclass receiver observations remain isolated.
- Exact function returns are inspected only for statically resolved local targets. Returned arrows retain the class receiver of their creation call and enter the existing bounded invocation graph when later called through aliases, optional calls, or bound forms. Arbitrary return values are not inferred.
- `Object.defineProperty`, `Object.defineProperties`, and `Reflect.set` now use the same normalized invocation model before contributing to ordered global-freeze/prototype mutation histories. Exact lexical replacement aliases on unresolved mutation targets fail closed; exact restoration and captured-safe aliases remain allowed.
- Focused GREEN passed provider-boundary 2/2, AI routing 167/167, full unit 311/311, all 14 lint targets, and the production boundary scan over 124 files. Migration verification passed empty database, previous `0005`, drift, rebuild equivalence, and DB integration 12/12; full integration passed 120/120. Forced verification passed task graph 77, secret scan 390, performance scan 95, formatting, boundaries 124, all 14 lint/typecheck/build targets, unit/coverage 311/311, and `git diff --check`.
- The first isolated full-integration attempt intentionally started PostgreSQL alone and exposed only missing Redis/MinIO/Keycloak listeners. It still passed migration verification and DB integration. The CI-equivalent rerun started all four services and passed; both runs removed temporary containers, networks, and volumes and restored Homebrew PostgreSQL 16.
- No database schema, migration, approved documentation, rubric, privacy mode, audit semantics, project-state file, or protected product rule changed. Fresh independent review remains required before T011 is approved and pushed.
