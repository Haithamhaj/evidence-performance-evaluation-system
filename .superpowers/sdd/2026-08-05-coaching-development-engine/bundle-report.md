...
- Forward migration `0032_coaching_development_integrity` adds append-only database triggers, validated current-revision ownership and source/AI-run/evidence references, and per-root resulting-version uniqueness. Persistence uses compare-and-swap writes; manager support is serialized and reauthorized inside its transaction.
- Action creation, revision, privacy and state transitions; plan creation, evidence linking, revision, agreement, activation, completion, withdrawal and closure; support; decisions; and protected reads append transactionally safe audit metadata without titles, reasons, notes, or support content.
...
- Focused PostgreSQL/Nest integration: 11 passed across 3 files, covering validated migration constraints, history mutation/deletion, orphan references, current pointers, duplicate versions, concurrent compare-and-swap, current manager authorization, actor-safe projections, exact safe audit receipts, create replay after state movement, cross-employee reference denial, formal-plan lifecycle, confirmed evidence, and seed rerun.
