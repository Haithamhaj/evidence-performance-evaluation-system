...
- Phase 0 Tasks T001–T013 are implemented and independently approved. Approved English rubric Version 1 is seeded as immutable, source-hashed infrastructure content with atomic bootstrap audit; the provider-neutral AI Router, bounded layered boundary controls, deterministic bilingual AI evaluation harness, and durable worker/queue foundation are complete; Arabic approval remains a separate T016 gate before employee-facing use.
...
Push the independently approved T013 worker/queue foundation, then execute T014 (evaluation eligibility snapshot foundation) using Subagent-Driven Development and TDD.
