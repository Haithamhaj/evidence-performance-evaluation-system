...
export class ResearchSourceReviewPersistence {
  ...
  async createOrReplayPending(input: CreatePendingReviewInput): Promise<SourceReviewLoaded> {
    return this.#database.$transaction(async (transaction) => {
      const existing = await transaction.researchSourceReview.findUnique({
        where: {
      ...
      return loadReview(transaction, input.id);
    }, serializable);
  }
    ...
    return this.#database.$transaction(async (transaction) => {
      ...
      if (review.state === input.terminalState && review.version === input.expectedVersion + 1) {
        const transitions = await transaction.researchProposalTransition.findMany({
          where: { proposal: { reviewId: input.reviewId }, reason: input.reason },
      ...
      if (input.terminalState === "CONFIRMED") {
        if (selected.size === 0 || proposals.some(({ id }) => selected.has(id)) === false) {
          throw proposalInvalid();
        }
        if ([...selected].some((id) => !proposals.some((proposal) => proposal.id === id))) {
          throw proposalInvalid();
        }
      ...
      await transaction.researchSourceReview.update({
        where: { id: input.reviewId },
        data: { state: input.terminalState, version: { increment: 1 } },
      });
      return loadReview(transaction, input.reviewId);
    }, serializable);
  }
