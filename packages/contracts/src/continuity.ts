...
export const LeaveRecordSchema = z
  .object({
    ...
    reasonCategory: LeaveReasonCategorySchema,
    affectedScopes: z.array(ContinuityScopeSchema).min(1).max(100),
...
export const LeaveDecisionSchema = z
  ...
  .strict();
export const LeaveTransitionSchema = z
  .object({
    schemaVersion: z.literal(1),
    id: UuidSchema,
    leaveId: UuidSchema,
    fromState: LeaveStateSchema,
    toState: LeaveStateSchema,
    actorId: UuidSchema,
...
export const HandoverItemSchema = z
  .object({
    ...
    currentState: requiredText(4_000),
    completedWork: requiredText(4_000),
    openWork: requiredText(4_000),
    blockersAndRisks: requiredText(4_000),
    immediateNextStep: requiredText(4_000),
    keyLinks: z.array(z.url()).max(50),
    requiredAccess: z.array(requiredText(500)).max(50),
    pendingDecisions: z.array(requiredText(2_000)).max(50),
    proposedDelegateId: UuidSchema.nullable(),
  })
  .strict();
export const HandoverRevisionSchema = z
  .object({
    schemaVersion: z.literal(1),
    id: UuidSchema,
    ...
    authorId: UuidSchema,
    items: z.array(HandoverItemSchema).min(1).max(100),
    createdAt: UtcSchema,
  })
  .strict();
...
export const DelegationScopeSchema = z
  .object({
    projectIds: z.array(UuidSchema).max(100),
    workstreamIds: z.array(UuidSchema).max(100),
    actions: z.array(ActingOwnerActionSchema).min(1),
  })
  .strict()
  .refine(({ projectIds, workstreamIds }) => projectIds.length + workstreamIds.length > 0, {
...
export const DelegationSchema = z
  .object({
    ...
    period: UtcIntervalSchema,
    scope: DelegationScopeSchema,
    emergency: z.boolean(),
    emergencyReason: requiredText(2_000).nullable(),
    version: VersionSchema,
  })
  .strict()
  .superRefine(({ emergency, emergencyReason }, context) => {
    ...
    if (!emergency && emergencyReason)
      context.addIssue({
        code: "custom",
        message: "emergency reason is not allowed",
        path: ["emergencyReason"],
      });
  });
export const DelegateConfirmationSchema = z
  .object({
    schemaVersion: z.literal(1),
    id: UuidSchema,
    ...
    confirmedAt: UtcSchema,
  })
  .strict();
export const DelegationAccessGapSchema = z
  .object({
    schemaVersion: z.literal(1),
    id: UuidSchema,
    delegationId: UuidSchema,
...
export const ReturnHandoverSchema = z
  .object({
    ...
    id: UuidSchema,
    delegationId: UuidSchema,
    actingOwnerId: UuidSchema,
    originalOwnerId: UuidSchema,
    completedWork: requiredText(4_000),
    decisionsAndChanges: requiredText(4_000),
    openWork: requiredText(4_000),
    risksAndNextSteps: requiredText(4_000),
    choice: ReturnChoiceSchema,
    confirmedById: UuidSchema.nullable(),
    createdAt: UtcSchema,
  })
  .strict();
...
export const ReassignmentRequiredCaseSchema = z
  .object({
    schemaVersion: z.literal(1),
    id: UuidSchema,
    formerOwnerId: UuidSchema,
    scope: ContinuityScopeSchema,
    state: ReassignmentCaseStateSchema,
    createdAt: UtcSchema,
  })
  .strict();
export const ReassignmentResolutionSchema = z
  .object({
    schemaVersion: z.literal(1),
    id: UuidSchema,
    caseId: UuidSchema,
    actorId: UuidSchema,
    actorRole: z.literal("MANAGER"),
    resolution: z.enum(["PERMANENT_REASSIGNMENT", "PAUSE", "CLOSE", "MERGE"]),
    successorId: UuidSchema.nullable(),
    effectiveAt: UtcSchema,
    reason: requiredText(2_000),
    createdAt: UtcSchema,
  })
  .strict();
export const DeactivationReceiptSchema = z
  .object({
    schemaVersion: z.literal(1),
    userId: UuidSchema,
    administratorId: UuidSchema,
    deactivatedAt: UtcSchema,
    preservedHistory: z.literal(true),
    reassignmentCaseIds: z.array(UuidSchema),
  })
  .strict();
export const RetentionReferenceSchema = z
  .object({
    schemaVersion: z.literal(1),
    policyVersionId: UuidSchema,
    archiveOnly: z.literal(true),
    automaticDeletion: z.literal(false),
  })
  .strict();
export const OperationalLeaveProjectionSchema = z
  .object({
    employeeId: UuidSchema,
    state: z.enum(["APPROVED", "ACTIVE", "RETURNED"]),
    startsAt: UtcSchema,
    endsAt: UtcSchema,
    checkInRequired: z.literal(false),
    negativeRegularitySignal: z.literal(false),
  })
  .strict();
