...
const formats = new Map<string, Readonly<{
  mediaClass: import("./model.js").UploadMediaClass;
  ...
  ["docx", { mediaClass: "office", mimes: ["application/vnd.openxmlformats-officedocument.wordprocessingml.document"], detectedExtensions: ["docx", "zip"] }],
  ...
  ["wav", { mediaClass: "audio", mimes: ["audio/wav", "audio/x-wav"], detectedExtensions: ["wav"] }],
  ["mp3", { mediaClass: "audio", mimes: ["audio/mpeg"], detectedExtensions: ["mp3"] }],
  ["m4a", { mediaClass: "audio", mimes: ["audio/mp4", "audio/x-m4a"], detectedExtensions: ["m4a", "mp4"] }],
]);
...
export async function inspectFile(input: InspectionInput): Promise<InspectedFile> {
  ...
  if (extension === "md" || extension === "txt") {
    ...
    return { detectedType: extension, detectedMime: input.declaredMime, mediaClass: "text", byteSize: file.size };
  }
  if (extension === "docx") {
    await inspectDocx(input.path, input.policy);
    return { detectedType: "docx", detectedMime: input.declaredMime, mediaClass: "office", byteSize: file.size };
  }
  const detected = await fileTypeFromFile(input.path);
  if (detected === undefined || !format.detectedExtensions.includes(detected.ext)) throw typeRejected();
  return {
...
async function assertUtf8(path: string): Promise<void> {
  ...
  try {
    for await (const chunk of createReadStream(path)) decoder.decode(chunk as Buffer, { stream: true });
    decoder.decode();
  } catch {
    throw typeRejected();
  }
        ...
        void (async () => {
          ...
          for await (const chunk of stream) {
            ...
            if (
              ...
              (entry.compressedSize > 0 && actual / entry.compressedSize > policy.maxArchiveCompressionRatio)
            ) throw safetyRejected();
          }
...
function openZip(path: string): Promise<yauzl.ZipFile> {
  return new Promise((resolve, reject) => {
    yauzl.open(path, { autoClose: true, lazyEntries: true, strictFileNames: true, validateEntrySizes: true }, (error, zip) => {
...
function typeRejected() { return new AppError("UPLOAD_TYPE_REJECTED", "errors.documents.uploadTypeRejected", 400); }
function sizeRejected() { return new AppError("UPLOAD_SIZE_REJECTED", "errors.documents.uploadSizeRejected", 413); }
function safetyRejected() { return new AppError("UPLOAD_SAFETY_REJECTED", "errors.documents.uploadSafetyRejected", 400); }
