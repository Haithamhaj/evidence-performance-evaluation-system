...
export class CoachingDevelopmentPersistence {
  ...
  async findInsight(id: string) {
    return (await this.database.coachingInsight.findUnique({ where: { id } })) as unknown as Record<
      string,
      unknown
    > | null;
  }
  ...
  async find(actionId: string) {
    return (await this.database.developmentAction.findUnique({
      where: { id: actionId },
    })) as unknown as {
      id: string;
      employeeId: string;
  ...
  async create(input: Record<string, unknown>) {
    return this.database.$transaction(async (tx) => {
      ...
      const revision = await tx.developmentActionRevision.create({
        data: revisionData(action.id, 1, input) as never,
      });
  ...
  async revise(input: Record<string, unknown>) {
    return this.database.$transaction(async (tx) => {
      ...
      const revision = await tx.developmentActionRevision.create({
        data: revisionData(action.id, action.version + 1, input) as never,
      });
      await tx.developmentAction.update({
        where: { id: action.id },
  ...
  async isAuthorizedManager(employeeId: string, managerId: string) {
    return (
      (await this.database.evaluationAssignment.count({
        where: { employeeId, managerId, employee: { active: true } },
      })) > 0
    );
  }
  async appendSupport(entry: Record<string, unknown>) {
    const action = await this.database.developmentAction.findUnique({
      where: { id: String(entry.actionId) },
    });
    if (
      !action ||
      action.privacy !== "SHARED" ||
      !(await this.isAuthorizedManager(action.employeeId, String(entry.managerId)))
    )
      throw failure("AUTHZ_SCOPE", 403);
    await this.database.managerSupportEntry.create({
      data: {
  ...
  async findPlan(planId: string) {
    return (await this.database.formalDevelopmentPlan.findUnique({
      where: { id: planId },
  ...
  async createPlan(event: Record<string, unknown>) {
    return this.database.$transaction(async (tx) => {
      ...
      const revision = await tx.formalDevelopmentPlanRevision.create({
        data: planRevisionData(plan.id, 1, event) as never,
      });
