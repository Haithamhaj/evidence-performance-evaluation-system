...
export const CoachingInsightAiOutputSchema = z
  .object({
    ...
    pattern: z.string().min(1).max(4_000),
    sourceIds: z.array(z.string().uuid()).min(1).max(100),
    confidence: z.enum(["SUPPORTED", "REVIEW_REQUIRED", "LIMITED"]),
    confidenceBasis: z.string().min(1).max(2_000),
    limitations: z.array(z.string().min(1).max(2_000)).min(1).max(20),
    conflicts: z.array(z.string().min(1).max(2_000)).max(20),
    cannotConclude: z.string().min(1).max(2_000),
    actionDraft: z
      .object({
        title: z.string().min(1).max(500),
        objective: z.string().min(1).max(4_000),
        activity: z.string().min(1).max(4_000),
      })
      .strict()
      .nullable(),
  })
...
export function buildCoachingInsightRequest(
  input: Readonly<{
