...
export class VoiceUpdatesController {
  ...
  constructor(voice: VoiceUpdateService) {
    this.voice = voice;
  }
  start(request: Request, body: unknown) {
    return this.voice.start({
      ...
      input: StartVoiceUpdateInputSchema.parse(body),
    });
  }
  revise(request: Request, voiceSessionId: string, body: unknown) {
    return this.voice.reviseTranscript({
      actor: actor(request),
      voiceSessionId: z.string().uuid().parse(voiceSessionId),
      input: ReviseVoiceTranscriptInputSchema.parse(body),
    });
  }
  ...
  cancel(request: Request, voiceSessionId: string) {
  ...
  retry(request: Request, voiceSessionId: string) {
    return this.voice.retry({
      actor: actor(request),
      correlationId: request.correlationId,
      voiceSessionId: z.string().uuid().parse(voiceSessionId),
    });
  }
  cancelByIdempotencyKey(request: Request, idempotencyKey: string) {
    return this.voice.cancelByIdempotencyKey({
      actor: actor(request),
      correlationId: request.correlationId,
      idempotencyKey: z.string().uuid().parse(idempotencyKey),
    });
  }
}
...
for (const [method, path, parameter] of [
  ...
  if (method === "revise" || method === "confirm")
    Body()(VoiceUpdatesController.prototype, method, 2);
