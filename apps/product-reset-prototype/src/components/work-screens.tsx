    ...
    <button className="workItemRow" onClick={() => openWorkItem(item.id)} type="button">
      ...
      <span className="workItemMain">
        ...
        <span className="workItemNext">
          {item.blockerReason ? `${copy(locale, "labels.blocker")}: ${text(item.blockerReason, locale)} · ` : ""}
...
export function InboxScreen() {
  ...
  const convert = (item: (typeof inboxItems)[number]) => {
    const template: WorkItem = {
        ...
        history: [{ label: { ar: "حُوّل من صندوق الوارد", en: "Converted from Inbox" }, at: new Date().toISOString(), actor: "هيثم الحاج" }],
    };
          ...
          {visible.map((item) => (
            <article className="inboxRow" key={item.id}>
              ...
              <div className="rowActions">
                ...
                <button className="secondaryButton" onClick={() => convert(item)} type="button">{copy(locale, "actions.convert")}</button>
                <button className="secondaryButton" onClick={() => setLinkedIds((current) => new Set([...current, item.id]))} type="button">{linkedIds.has(item.id) ? (locale === "ar" ? "تم الربط" : "Linked") : copy(locale, "actions.link")}</button>
        ...
        {projects.map((project) => {
          ...
          return (
            <button className="surface projectCard" key={project.id} onClick={() => navigate(`projects/${project.id}`)} type="button">
              ...
              <p className="projectRole">{copy(locale, "labels.role")}: {text(project.employeeRole, locale)}</p>
              <dl>
                <div><dt>{locale === "ar" ? "مسارات العمل" : "Workstreams"}</dt><dd>{streamCount}</dd></div>
                <div><dt>{copy(locale, "labels.openItems")}</dt><dd>{progress.total - progress.completed}</dd></div>
              </dl>
              <div className="progressTrack" aria-label={`${progress.percent}%`}><span style={{ inlineSize: `${progress.percent}%` }} /></div>
              <span className="nextLine"><b>{copy(locale, "labels.latestUpdate")}:</b> {text(project.latestUpdate, locale)}</span>
              <span className="nextLine"><b>{copy(locale, "labels.nextAction")}:</b> {text(project.nextAction, locale)}</span>
              {project.blocker ? <span className="nextLine riskText"><b>{copy(locale, "labels.blocker")}:</b> {text(project.blocker, locale)}</span> : null}
            </button>
          );
        })}
      </div>
    ...
    <>
      ...
      {tab === "overview" ? (
        <div className="detailGrid">
          <section className="surface detailCard"><h2>{copy(locale, "labels.latestUpdate")}</h2><p>{text(project.latestUpdate, locale)}</p><h3>{locale === "ar" ? "المراحل" : "Milestones"}</h3>{project.milestones.map((milestone) => <p className="criterionLine" key={milestone.en}>◇ {text(milestone, locale)}</p>)}<h3>{copy(locale, "labels.nextAction")}</h3><p>{text(project.nextAction, locale)}</p>{project.blocker ? <div className="riskCallout"><b>{copy(locale, "labels.blocker")}</b><p>{text(project.blocker, locale)}</p></div> : null}</section>
      ...
      {tab === "overview" ? <section className="workGroup"><div className="sectionHeading"><h2>{locale === "ar" ? "مسارات العمل" : "Workstreams"}</h2><span>{streams.length}</span></div><div className="projectGrid">{streams.map((stream) => <button className="surface projectCard" key={stream.id} onClick={() => navigate(`projects/${projectId}/workstreams/${stream.id}`)} type="button"><StatusBadge kind="health" value={stream.health} /><h2>{text(stream.name, locale)}</h2><p>{text(stream.purpose, locale)}</p><span>{stream.owner}</span></button>)}</div></section> : null}
      {!["overview", "work"].includes(tab) ? <section className="surface detailCard"><h2>{locale === "ar" ? tabs.find(([id]) => id === tab)?.[1] : tabs.find(([id]) => id === tab)?.[2]}</h2><p>{locale === "ar" ? "محتوى واقعي للمعاينة ضمن النموذج؛ لا توجد كتابة إلى النظام الإنتاجي." : "Realistic preview content; no production-system writes are made."}</p>{tab === "people" ? streams.map((stream) => <div className="responsibility" key={stream.id}><strong>{stream.owner}</strong><span>{text(stream.name, locale)}</span><small>{stream.contributors.join(" · ")}</small></div>) : null}</section> : null}
    </>
  );
}
  ...
  return (
    <>
      ...
      <div className="detailHero">
        ...
        <dl><div><dt>{copy(locale, "labels.project")}</dt><dd>{project ? text(project.name, locale) : ""}</dd></div><div><dt>{copy(locale, "labels.owner")}</dt><dd>{stream.owner}</dd></div><div><dt>{locale === "ar" ? "المساهمون" : "Contributors"}</dt><dd>{stream.contributors.join("، ")}</dd></div><div><dt>{locale === "ar" ? "المخرج المستهدف" : "Target output"}</dt><dd>{text(stream.targetOutput, locale)}</dd></div></dl>
      </div>
      <div className="detailGrid">
        ...
        <section className="surface detailCard"><h2>{locale === "ar" ? "المؤشرات التشغيلية والمسؤولية" : "Operational KPIs and responsibility"}</h2>{stream.kpis.map((kpi) => <div className="kpiLine" key={kpi.label.en}><span>{text(kpi.label, locale)}</span><strong>{kpi.value}</strong></div>)}{stream.responsibilityHistory.map((entry) => <div className="responsibility" key={entry.startsAt}><strong>{entry.person}</strong><span>{text(entry.role, locale)}</span><small>{entry.startsAt} — {entry.endsAt ?? (locale === "ar" ? "مستمر" : "ongoing")}</small></div>)}</section>
      </div>
      <section className="surface detailCard workstreamContext"><h2>{locale === "ar" ? "التحديثات والأدلة والمستند" : "Updates, evidence, and document"}</h2><p>{locale === "ar" ? "آخر تحديث محفوظ قرب سجل العمل، مع دليل مصدر ومستند عمل ذي إصدارات محفوظة." : "Latest update stays near the work record, with source evidence and a versioned work document."}</p></section>
...
export function WorkItemPanel() {
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === "Tab" && panelRef.current) {
        const controls = [...panelRef.current.querySelectorAll<HTMLElement>("button, a, input, select, textarea, [tabindex]:not([tabindex='-1'])")].filter((control) => !control.hasAttribute("disabled"));
  ...
  return createPortal(
    <div className="panelBackdrop" onMouseDown={closeWorkItem}>
      <aside aria-labelledby="work-item-title" aria-modal="true" className="workItemPanel" onMouseDown={(event) => event.stopPropagation()} ref={panelRef} role="dialog">
        ...
        <div className="panelBody">
          ...
          <dl className="propertyGrid">
            ...
            <div><dt>{locale === "ar" ? "المشاركون" : "Participants"}</dt><dd>{item.participants.join("، ")}</dd></div>
            <div><dt>{locale === "ar" ? "النوع" : "Type"}</dt><dd>{item.type}</dd></div>
            <div><dt>{locale === "ar" ? "تاريخ البدء" : "Start date"}</dt><dd>{dateLabel(item.startDate, locale)}</dd></div>
          ...
          <section><h3>{locale === "ar" ? "المتطلبات ومعايير القبول" : "Requirements and acceptance criteria"}</h3>{item.requirements.map((requirement) => <p className="criterionLine" key={requirement.en}>◇ {text(requirement, locale)}</p>)}{item.acceptanceCriteria.map((criterion) => <p className="criterionLine" key={criterion.en}>□ {text(criterion, locale)}</p>)}</section>
          <section><h3>{locale === "ar" ? "الروابط والسياق" : "Links and context"}</h3><p><b>{locale === "ar" ? "الاعتماديات:" : "Dependencies:"}</b> {item.dependencies.join("، ") || "—"}</p><p><b>{locale === "ar" ? "المعايير المرتبطة:" : "Related criteria:"}</b> {item.criterionIds.join("، ") || "—"}</p><p><b>GitHub:</b> {item.githubLinks.join("، ") || "—"}</p><p><b>{locale === "ar" ? "سياق المساهمة:" : "Contribution context:"}</b> {text(item.contributionContext, locale)}</p></section>
          <section><h3>{locale === "ar" ? "التحديثات والتعليقات" : "Updates and comments"}</h3><p>{item.updateIds.length ? item.updateIds.join("، ") : (locale === "ar" ? "لا يوجد تحديث مؤكد بعد." : "No confirmed update yet.")}</p><p className="mutedText">{locale === "ar" ? "تعليقات الفريق تظهر هنا مع الهوية والتوقيت." : "Team comments appear here with identity and timestamp."}</p></section>
