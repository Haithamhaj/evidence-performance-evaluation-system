...
export const ANALYSIS_CRITERIA_SECRET_RESOLVER = Symbol(
  "ANALYSIS_CRITERIA_SECRET_RESOLVER",
);
...
export class WorkerEnvironmentAiCredentialSecretResolver {
  ...
  async get(providerKey: string): Promise<string | undefined> {
    ...
    const value = this.environment[`AI_PROVIDER_${normalized}_API_KEY`]?.trim();
...
export function createWorkerRuntimeAiRouter(
  ...
  import("@evaluation/ai-routing").AiRouter<
