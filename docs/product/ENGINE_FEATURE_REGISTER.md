...
**Authoritative sources:** `PROJECT_REFERENCE.md` §§13, 15, 25, 35; rubric ARL-01/02; approved engine-first decision | **Owner module:** planned bounded Research & Experiments domain; existing Projects, Work Items, Documents, Connected Work Context, Updates & Evidence, and AI Router remain source owners | **Inputs/sources:** Project/Workstream context snapshot, documents, Tasks, explicit links, papers/repos/notes/evidence | **AI role:** citation-bound source relevance review, draft framing, compare sources, expose uncertainty
**AI prohibitions:** source volume is not quality or performance; no rating | **Human gate:** employee confirms framing/synthesis/decision | **States/transitions:** draft → active → concluded/superseded with append-only revisions | **Public API/events:** none yet | **Authorization/privacy:** must follow Project scope and private-source rules | **Audit/history:** required but absent | **Failure/recovery:** required but absent | **Tests:** authoritative rubric fixtures only; no production domain tests
**Status:** PLANNED | **External gate:** private/licensed source access may require existing approved connector authorization | **Frontend implications:** paste a link or question, receive a compact cited relevance review, and confirm editable Research/Experiment/Task proposals inside one workspace—not a generic knowledge platform
...
**Authoritative sources:** rubric ARL-03; `PROJECT_REFERENCE.md` §§13, 15, 25; approved engine-first decision | **Owner module:** planned bounded Research & Experiments domain; Documents and Updates & Evidence remain artifact/evidence owners | **Inputs/sources:** research question, datasets, models, code, benchmarks, artifacts | **AI role:** draft method/check completeness/summarize results
**AI prohibitions:** experiment count is not score; AI cannot confirm conclusion as fact | **Human gate:** employee confirms method, result, conclusion, and decision | **States/transitions:** draft → running → result recorded → concluded/abandoned/superseded | **Public API/events:** none yet | **Authorization/privacy:** Project scope, upload safety, model/data privacy required | **Audit/history:** required but absent | **Failure/recovery:** resumable/incomplete experiment handling required | **Tests:** no production tests
**Status:** PLANNED | **External gate:** dataset/model access may be experiment-specific | **Frontend implications:** progressive method template; do not force heavy form completion before work begins
...
**Authoritative sources:** rubric ARL-04; `PROJECT_REFERENCE.md` §§25, 35 | **Owner module:** future Research & Experiments boundary with Timeline/Evidence public interfaces | **Inputs/sources:** CAP-025/026 conclusions and subsequent Task/Update/decision | **AI role:** suggest traceable links and missing application
**AI prohibitions:** unapplied learning cannot be inferred as applied; no performance judgment | **Human gate:** employee confirmation | **States/transitions:** proposed application → confirmed/rejected → superseded | **Public API/events:** none yet | **Authorization/privacy:** same Project/responsibility scopes | **Audit/history:** required but absent | **Failure/recovery:** unresolved links remain gaps | **Tests:** no production tests
**Status:** PLANNED | **External gate:** none | **Frontend implications:** show “what changed because of this?” as a lightweight next action
...
**Authoritative sources:** protected rules 7, 10–13, 21; `PROJECT_REFERENCE.md` §§17–20; T010, T014, T045–T046 | **Owner module:** seed/database foundation exists; bounded evaluation domain not yet complete | **Inputs/sources:** approved rubric, cadence, participants, leave, visibility mode | **AI role:** none for activation/snapshot
**AI prohibitions:** cannot change templates, eligibility, or visibility | **Human gate:** authorized cycle creation/activation | **States/transitions:** template draft/active/retired; cycle draft/open/closed; snapshot immutable | **Public API/events:** no full cycle API | **Authorization/privacy:** Identified pilot mode frozen per cycle | **Audit/history:** rubric and eligibility seed/foundation exist; complete cycle snapshot absent | **Failure/recovery:** atomic final snapshot/retry still required | **Tests:** rubric seed, eligibility and identified-visibility contract tests; migration `0005`, `0008`
**Status:** PARTIAL | **External gate:** Arabic rubric remains T016 gate | **Frontend implications:** cycle setup is administrative; employees see cycle type and rules, not configuration internals
...
**Authoritative sources:** `PROJECT_REFERENCE.md` §§18–21; T048, T052 | **Owner module:** future evaluation domain | **Inputs/sources:** immutable cycle snapshot, CAP-024 Fact View, employee interpretation | **AI role:** justification help only after the employee selects a rating
**AI prohibitions:** no suggested/challenged/normalized rating | **Human gate:** employee rating selection and submission | **States/transitions:** not-started → draft → submitted/locked | **Public API/events:** none yet | **Authorization/privacy:** employee own assessment; manager-draft independence preserved | **Audit/history:** immutable submission required | **Failure/recovery:** draft recovery and one protected submit required | **Tests:** no production workflow tests
**Status:** PLANNED | **External gate:** CAP-028 | **Frontend implications:** facts first, then one criterion at a time; AI helps wording only after choice
...
**Authoritative sources:** protected rules 1, 9; `PROJECT_REFERENCE.md` §§20–21; T049 | **Owner module:** future evaluation domain | **Inputs/sources:** cycle snapshot, manager-authorized Fact View, direct observation | **AI role:** source organization and post-rating justification drafting only
**AI prohibitions:** no rating recommendation/prediction, no readiness percentage, no automatic average | **Human gate:** manager selects and submits every rating | **States/transitions:** independent draft → submitted/locked | **Public API/events:** none yet | **Authorization/privacy:** employee self-rating hidden until manager draft submission | **Audit/history:** draft independence and submission timestamp required | **Failure/recovery:** recover draft without revealing self-rating | **Tests:** no production workflow tests
**Status:** PLANNED | **External gate:** CAP-028 | **Frontend implications:** neutral evidence and anchors; manager remains visibly responsible for the decision
...
**Authoritative sources:** protected rules 7, 9; `PROJECT_REFERENCE.md` §§21–22; T050–T051 | **Owner module:** future evaluation domain | **Inputs/sources:** submitted CAP-029/030 assessments | **AI role:** neutral comparison/justification drafting after ratings exist
**AI prohibitions:** no compromise/recommended rating; reservation never changes manager rating | **Human gate:** discussion, manager finalization, employee acknowledgment/reservation | **States/transitions:** compared → discussed → finalized → acknowledged/reserved → closed | **Public API/events:** none yet | **Authorization/privacy:** only cycle participants and authorized manager | **Audit/history:** closed evaluation immutable; formal future revision only | **Failure/recovery:** atomic finalization and idempotent acknowledgment required | **Tests:** no production workflow tests
**Status:** PLANNED | **External gate:** CAP-028–030 | **Frontend implications:** explain differences without nudging a midpoint; clear final decision ownership
...
**Authoritative sources:** `PROJECT_REFERENCE.md` §35; T053 | **Owner module:** future evaluation/reporting boundary | **Inputs/sources:** closed snapshots, sources, visibility mode | **AI role:** optional approved narrative summary only
**AI prohibitions:** no ranking, inferred rating, or private-mode leakage | **Human gate:** authorized generation/download | **States/transitions:** requested → generated/failed/expired | **Public API/events:** none yet | **Authorization/privacy:** report-specific field allowlists | **Audit/history:** generation/access audit and snapshot identity required | **Failure/recovery:** reproducible retry and expired-link recovery | **Tests:** no production export tests; Arabic PDF/DOCX fixtures remain required
**Status:** PLANNED | **External gate:** Arabic content gate for Arabic evaluation exports | **Frontend implications:** explicit report audience, locale, content preview, and safe download
...
**Status:** PARTIAL | **External gate:** production telemetry/alert destination | **Frontend implications:** admin-only concise status and recovery guidance; no raw logs in normal UI
...
**AI prohibitions:** no secret/private-mode leakage | **Human gate:** sensitive access and retention policy approval | **States/transitions:** controls are partly implemented; release review unresolved | **Public API/events:** cross-cutting | **Authorization/privacy:** many foundations complete; future modes/retention execution incomplete | **Audit/history:** protected access and mutations require audit | **Failure/recovery:** incident/revocation/retention procedures still required | **Tests:** security boundary, secret scan, upload, authorization, AI-boundary tests
**Status:** PARTIAL | **External gate:** future privacy-mode approval and production secrets/config | **Frontend implications:** privacy explanations and permission errors must reflect actual enforcement
...
**Authoritative sources:** T075; migration/retention rules | **Owner module:** future operational procedure across PostgreSQL/object storage/config | **Inputs/sources:** database and object snapshots | **AI role:** none
**AI prohibitions:** none applicable | **Human gate:** authorized destructive restore procedure | **States/transitions:** backup requested/completed/verified; restore drill completed | **Public API/events:** none | **Authorization/privacy:** encrypted, access-controlled backups | **Audit/history:** drill evidence and recovery point retained | **Failure/recovery:** documented rollback and integrity validation required | **Tests:** no restore drill
**Status:** PLANNED | **External gate:** production backup target/credentials | **Frontend implications:** admin-only status; restore remains protected human operation
...
**AI prohibitions:** all protected rules remain release blockers | **Human gate:** Product Owner launch acceptance and unavoidable external administrators | **States/transitions:** not-ready → dry-run-ready → pilot-approved → launched/rolled-back | **Public API/events:** system-wide | **Authorization/privacy:** full role journey verification | **Audit/history:** release evidence and configuration snapshot required | **Failure/recovery:** rollback/restore/support runbook required | **Tests:** current suites are foundation; final dry run not executed
**Status:** PLANNED | **External gate:** Google/GitHub/identity/storage/email/backup production setup | **Frontend implications:** final frontend program and user acceptance must finish before launch
